...
public extension AVAudioPCMBuffer {
                                 ...
                                 frames: AVAudioFrameCount = 0) -> AVAudioFrameCount
    {
        ...
        if remainingCapacity == 0 {
            debugPrint("AVAudioBuffer copy(from) - no capacity!")
            return 0
        }

        if format != buffer.format {
            debugPrint("AVAudioBuffer copy(from) - formats must match!")
            return 0
        }
        ...
        if totalFrames <= 0 {
            debugPrint("AVAudioBuffer copy(from) - No frames to copy!")
            return 0
        }
